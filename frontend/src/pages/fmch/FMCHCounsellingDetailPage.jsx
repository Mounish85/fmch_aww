...
import { ArrowLeft, CheckCircle2, Clock, MessageSquare, ShieldCheck } from 'lucide-react';
import LoadingState from '../../components/common/LoadingState';
import ErrorMessage from '../../components/common/ErrorMessage';
import RecommendationCard from '../../components/counselling/RecommendationCard';
import { useLanguage } from '../../context/LanguageContext';

const FMCHCounsellingDetailPage = () => {
  const { id } = useParams();
  ...
  const fetchRecord = useCallback(async () => {
    try {
      ...
      setError('Counselling session record could not be loaded.');
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  if (loading) {
    return <LoadingState message={t('loading', 'LOADING SUPERVISORY RECORD...')} />;
  }
  ...
  const workerEmail = typeof conductedBy === 'object' ? conductedBy?.email : null;
  const workerLang = typeof conductedBy === 'object' ? conductedBy?.preferredLanguage : 'en';
      ...
      <div className="border-[3px] border-[#1a1a2e] shadow-[4px_4px_0_#1a1a2e] bg-white overflow-hidden mb-6">
        <div className="bg-[#1a1a2e] text-white px-6 py-4 flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="text-[10px] font-black uppercase bg-[#2ecc71] text-[#1a1a2e] px-2 py-0.5 inline-block mb-1">
                FMCH SUPERVISORY RECORD #{id.slice(-6).toUpperCase()}
              </span>
              <h1 className="text-xl sm:text-2xl font-black uppercase tracking-tight">
                {t('audit_view_title', 'COUNSELLING AUDIT VIEW')}
              </h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y-[3px] sm:divide-y-0 sm:divide-x-[3px] divide-[#1a1a2e] border-b-[3px] border-[#1a1a2e] bg-[#fafaf5]">
          <div className="p-6">
            <span className="text-xs font-black uppercase text-gray-500 block mb-1">
              {t('beneficiary_details_label', 'BENEFICIARY DETAILS')}
            </span>
            ...
            {beneficiaryPhone && (
              <p className="text-xs font-bold text-gray-700">{t('phone_label', 'Phone')}: {beneficiaryPhone}</p>
            )}
            {beneficiaryAddress && (
              <p className="text-xs font-bold text-gray-700">{t('address_label', 'Location')}: {beneficiaryAddress}</p>
            )}
          </div>

          <div className="p-6">
            <span className="text-xs font-black uppercase text-gray-500 block mb-1">
              {t('conducted_by_header', 'CONDUCTED BY')}
            </span>
            <h3 className="font-black text-lg text-[#1a1a2e] uppercase mb-1">
              {workerName}
            </h3>
            {workerEmail && (
              <p className="text-xs font-bold text-gray-700">{t('email_label', 'Email')}: {workerEmail}</p>
            )}
            <p className="text-xs font-bold text-gray-700">{t('preferred_lang_label', 'Language')}: {workerLang.toUpperCase()}</p>
            {formattedDate && (
              <p className="text-xs font-bold text-gray-700 mt-2">{t('recorded_at_label', 'Recorded')}: {formattedDate}</p>
            )}
          </div>
        </div>
        ...
        <div className="p-6">
          <h4 className="font-black text-sm uppercase tracking-tight text-[#1a1a2e] mb-2 flex items-center gap-2">
            ...
            {t('field_inputs_header', 'FIELD INPUTS & CONTEXT')}
          </h4>
      ...
      {recommendation ? (
        ...
        <div className="border-[3px] border-[#1a1a2e] bg-white p-6 text-center">
          <p className="font-black text-sm uppercase text-gray-600">
            {t('no_data_available', 'NO RECOMMENDATION RECORDED FOR THIS SESSION')}
          </p>
        </div>
      )}
    </div>
...
export default FMCHCounsellingDetailPage;
